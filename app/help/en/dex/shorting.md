...
In order to increase your exposure to RVP and offer liquidity to BitAssets, such
...
The RevPop network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 RVP as it is traded on external exchanges.
...
The RevPop network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/RVP).
...
market rates up to the SQP and close the position. The remaining RVP of the
...
increase, an additional amount of RVP is locked as collateral, while reducing
...
particular bitAsset to hand it over to the RevPop network. After that, the
